...
src/
  lib/
    ...
    storage/
      ...
      indexeddb.ts    # phase 1 implementation
...
- `domain/` imports nothing from `storage/` and never references `IDBDatabase`
  or `fetch`.
...
onto both IndexedDB and Cloudflare KV without a query engine.
...
IndexedDB layout: object store `cards` keyed on `id`; object store `purchases`
keyed on `id` with an index on `[cardId, date]`; object store `payments` keyed on
`[cardId, period]`. A single database version constant, with an upgrade handler
in place from the start.

## User Interface

The existing stack is used as-is: `@kctools/bun-server` in MPA mode, Lit
components, Pico CSS. No routing library and no custom design system. Three
pages, one query parameter.
...
- IndexedDB being unavailable (private window, storage disabled) is detected at
  startup and reported plainly, rather than silently discarding data.
...
- A single repository contract suite runs against both the in-memory and
  IndexedDB implementations (happy-dom is already a dependency), so the phase 2
  HTTP implementation inherits a ready conformance suite.

## Phases

### Phase 1 — local

The full application above, backed by IndexedDB. Usable offline with no account
and no network. JSON export and import ship in this phase: it is both the
...
POST   /api/cards/:id/purchases            put  purchase:<cardId>:<date>:<ulid>
